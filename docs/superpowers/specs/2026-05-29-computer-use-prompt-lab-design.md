...
case:                        # injected into prompt templates + used by /control/setup
  ...
  recommended_action: "Approve free-of-charge replacement of the rear alloy wheel."
  uuid: "A1B2C3"
expected_record:             # ground truth for scoring (matches the app's JSON schema)
...
> **Ground truth note.** `expected_record` mirrors the **actual app code**, not the older
> spec doc. The live apps use the Evora/wheels narrative (e.g. `Robertson, J.`,
> `Evora Alloy Wheel AW-200`, `EQ-2026-0042`, Status `Replacement Approved`); the
> implementer reads the field names/options from `warranty_case_manager.py` and
> `returns_portal/app.py` when authoring scenarios.
